...
public class ReviewAddController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (u != null) {
            if (!reviewDetail.trim().equals("")) {
                if (reviewDAO.createReview(u.getUserId(), productID, reviewDetail)) {
                    response.sendRedirect("productDetails?productID=" + productID);
                } else {

                }
            } else {
                session.setAttribute("notifiStt", false);
                //request.setAttribute("notifiStt", false);
                //request.getRequestDispatcher("productDetails.jsp").forward(request, response);
                response.sendRedirect("productDetails?productID=" + productID);
                System.out.println("thongba0 " + request.getParameter("notifiStt"));
            }
        } else {
